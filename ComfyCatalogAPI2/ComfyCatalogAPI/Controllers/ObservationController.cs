...
namespace ComfyCatalogAPI.Controllers
{
    ...
    public class ObservationController : Controller
    {
        ...
        [SwaggerResponse(StatusCodes.Status500InternalServerError, Description = "An unexpected API error has occurred.")]
        [HttpGet]
        [Route("/api/GetObservationByID/{obsID}")]
        public async Task<IActionResult> GetObservationByID(int obsID)
        {
            string CS = _configuration.GetConnectionString("WebApiDatabase");
            Response response = await ObservationLogic.GetObservationByID(CS, obsID);
            if (response.StatusCode != ComfyCatalogBLL.Utils.StatusCodes.SUCCESS)
            {
                return StatusCode((int)response.StatusCode);
        ...
        [SwaggerResponse(StatusCodes.Status500InternalServerError, Description = "An unexpected API error has occurred.")]
        [HttpGet]
        [Route("/api/GetObservationByUserID/{userID}")]
